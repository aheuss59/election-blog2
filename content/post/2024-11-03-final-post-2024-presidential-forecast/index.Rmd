...
$$ 
y_{ij} = \beta_0 + \beta voteshare_{ij-1} + \beta voteshare_{ij-2} + \beta incumbentparty_{j} + \beta Q2GDP_{j} + \beta unemployment_{ij} + \beta partyswing_{j} + \alpha_{i} + \epsilon_{ij}
$$
...
all_preds |>
  ...
  arrange(Winner) |>
  knitr::kable(caption="Swing State Prediction Breakdown", digits=4)
```
